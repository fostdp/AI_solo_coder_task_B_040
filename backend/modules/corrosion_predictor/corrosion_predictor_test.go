...
func TestNewCorrosionPredictor(t *testing.T) {
	...
	cp := NewCorrosionPredictor(cfg)
	if cp == nil {
		t.Fatal("NewCorrosionPredictor returned nil")
	}
	if cp.cfg != cfg {
		t.Error("Config not set correctly")
	}
	if cp.IsRunning() {
		t.Error("Predictor should not be running initially")
	}
	if cp.GreyModelService == nil {
		t.Error("GreyModelService should be initialized")
	}
}
...
func TestGreyModelPredictionAccuracy(t *testing.T) {
	...
	predictedRate, confidence := cp.greyModelPrediction(history, current)
...
func TestExponentialDecayPredictionAccuracy(t *testing.T) {
	...
	predictedRate, confidence := cp.exponentialDecayPrediction(history, current)
...
func TestPredictionVsActualDeviation(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			greyPred, _ := cp.greyModelPrediction(history, lastData)
			expPred, _ := cp.exponentialDecayPrediction(history, lastData)
...
func TestDifferentCorrosionEnvironments(t *testing.T) {
	...
	for _, env := range environments {
		t.Run(env.name, func(t *testing.T) {
			...
			greyPred, greyConf := cp.greyModelPrediction(history, lastData)
			expPred, expConf := cp.exponentialDecayPrediction(history, lastData)
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			for _, d := range tc.history[:len(tc.history)-1] {
				cp.storeInspectionData(d)
			}

			current := tc.history[len(tc.history)-1]
			cp.calculateCorrosionRate(current)
...
func TestThicknessPredictionGeneration(t *testing.T) {
	...
	predictions, months := cp.generateThicknessPredictions(data, 0.2)
...
func TestNextInspectionInterval(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			nextDate := cp.calculateNextInspection(data)
...
func TestPredictorLifecycle(t *testing.T) {
	...
	if cp.IsRunning() {
		t.Error("Predictor should not be running before Start")
	}
	...
	if !cp.IsRunning() {
		t.Error("Predictor should be running after Start")
	}

	cp.Start(ctx)
	if !cp.IsRunning() {
		t.Error("Predictor should still be running after second Start call")
	}
	...
	if cp.IsRunning() {
		t.Error("Predictor should not be running after Stop")
	}

	cp.ProcessInspectionData(data)

	statsAfterStop := cp.GetStats()
...
func TestRepairEventDetection(t *testing.T) {
	...
	cp.mu.RLock()
	history := cp.pipeData["PIPE-001"]
	cp.mu.RUnlock()

	if len(history) != 3 {
		t.Fatalf("应存在3条历史数据，实际有%d条", len(history))
	}

	beforeLastThickness := history[len(history)-1].CurrentWallThickness
	...
	detected := cp.detectRepairEvent(repairData)
	if !detected {
		t.Fatal("应检测到管道修复事件，但未检测到")
	}

	cp.mu.RLock()
	repairs, hasRepairs := cp.repairEvents["PIPE-001"]
	cp.mu.RUnlock()
	...
	expectedGain := 11.5 - beforeLastThickness
	if repair.ThicknessGain != expectedGain {
		t.Errorf("壁厚增益计算错误: %.1f, 期望: %.1f",
			repair.ThicknessGain, expectedGain)
	}

	gainRatio := repair.ThicknessGain / repairData.OriginalWallThickness
	if gainRatio <= cfg.RepairThresholdRatio {
		t.Errorf("增益比率应大于阈值: %.3f <= %.3f", gainRatio, cfg.RepairThresholdRatio)
	}

	t.Logf("修复事件检测成功 - 壁厚增益:%.1fmm, 增益比率:%.1f%%, 重置前基线:%.1fmm",
		repair.ThicknessGain, gainRatio*100, beforeLastThickness)
}
...
func TestModelResetAfterRepair(t *testing.T) {
	...
	cp.mu.RLock()
	beforeResetHistory := cp.pipeData["PIPE-001"]
	beforeDataCount := len(beforeResetHistory)
	beforeLastThickness := beforeResetHistory[len(beforeResetHistory)-1].CurrentWallThickness
	beforeResetCount := cp.modelResetCount
	cp.mu.RUnlock()
	...
	cp.ProcessInspectionData(repairData)

	cp.mu.RLock()
	afterResetHistory, exists := cp.pipeData["PIPE-001"]
	afterResetCount := cp.modelResetCount
	cp.mu.RUnlock()

	if !exists {
		t.Fatal("模型重置后应仍存在管道数据")
	}

	afterLastThickness := afterResetHistory[len(afterResetHistory)-1].CurrentWallThickness
	if afterLastThickness != 11.0 {
		t.Errorf("模型基线应重置为修复后的壁厚: %.1f, 实际: %.1f",
			11.0, afterLastThickness)
	}

	if len(afterResetHistory) >= beforeDataCount {
		t.Errorf("历史数据应被清理: %d >= %d",
			len(afterResetHistory), beforeDataCount)
	}

	if len(afterResetHistory) < 1 {
		t.Error("重置后应至少保留一条最新数据")
	}

	if afterResetCount <= beforeResetCount {
		t.Error("重置计数应增加")
	}

	t.Logf("模型重置成功 - 重置前基线:%.1fmm → 重置后基线:%.1fmm, 历史数据:%d→%d条, 重置次数:%d→%d",
		beforeLastThickness, afterLastThickness,
		beforeDataCount, len(afterResetHistory), beforeResetCount, afterResetCount)
}
...
func TestModelResetCoolDown(t *testing.T) {
	...
	cp.ProcessInspectionData(initialData)
	...
	cp.ProcessInspectionData(firstRepair)

	cp.mu.RLock()
	firstResetCount := cp.modelResetCount
	cp.mu.RUnlock()
	...
	cp.ProcessInspectionData(secondRepair)

	cp.mu.RLock()
	secondResetCount := cp.modelResetCount
	cp.mu.RUnlock()
	...
	cp.ProcessInspectionData(thirdRepair)

	cp.mu.RLock()
	thirdResetCount := cp.modelResetCount
	cp.mu.RUnlock()
...
func TestSmallThicknessGainIgnored(t *testing.T) {
	...
	cp.ProcessInspectionData(data1)
	...
	detected := cp.detectRepairEvent(smallGainData)
	if detected {
		t.Error("小幅壁厚增益不应触发修复检测")
	}

	cp.mu.RLock()
	resetCount := cp.modelResetCount
	cp.mu.RUnlock()
